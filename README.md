# 最近の変更
2021年1月に下記の設定を行いました。
入力項目が多すぎて使いづらいというお声を頂いたため、タンパク質、脂質、炭水化物の入力項目を削除しよりシンプルにしました。
投稿詳細画面にページビュー数を表示しました。自分の頑張りが多くの人に見られているとわかれば、よりモチベーションにつながると考えたためです。

# 概要
自分がかつて「減量が続かない」という悩みを抱えていた経験から、「減量を継続して成功させたい」ユーザー様のためのアプリを開発しました。    
私自身このアプリを使い2ヶ月で４キロの減量に成功しました。
